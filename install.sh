...
Options:
  ...
  --db-port PORT        PostgreSQL port (default: 8888)
  --db-name DB            database name (default: falcoclaw)
  --help                       Show this help
...
EOSQL
    ...
    if command -v psql &>/dev/null; then
        echo "$SQL" | psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -U postgres 2>/dev/null && \
            ...
            warn "Could not connect to database. Schema SQL saved to config/falcoclaw_schema.sql"
    else
        warn "psql not found. Schema SQL saved to config/falcoclaw_schema.sql"
    fi

    echo "$SQL" > "${REPO_DIR}/config/falcoclaw_schema.sql"
}
...
# Routes Falco alerts to Telegram and (PostgreSQL)
...
# PostgreSQL — durable alert storage for database
postgresql:
  host: "${DB_HOST}"
  port: "${DB_PORT}"
  database: "${DB_NAME}"
