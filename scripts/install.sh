...
Options:
  ...
  --db-port PORT        Database PostgreSQL port (default: 8888)
  --db-name DB            database name (default: database)
  --help                       Show this help
...
setup_db_schema() {
    log "Setting up FalcoClaw alerts table in Database..."
...
-- View for unresolved critical alerts (for responders)
...
EOSQL
    ...
    if command -v psql &>/dev/null; then
        echo "$SQL" | psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -U postgres 2>/dev/null && \
            log "Database schema created successfully." || \
            warn "Could not connect to Database. Schema SQL saved to config/database_schema.sql"
    else
        warn "psql not found. Schema SQL saved to config/database_schema.sql"
    fi

    echo "$SQL" > "${REPO_DIR}/config/database_schema.sql"
}
...
# Routes Falco alerts to Telegram and Database (PostgreSQL)
...
# PostgreSQL — durable alert storage in Database
postgresql:
  host: "${DB_HOST}"
  port: "${DB_PORT}"
  database: "${DB_NAME}"
